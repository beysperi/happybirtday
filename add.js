...
function updateCountdown() {
    ...
    if (now.getMonth() > 5 || (now.getMonth() === 5 && now.getDate() > 11)) {
        birthdayYear = currentYear + 1;
    }

    const birthday = new Date(birthdayYear, 5, 11); // Month is 0-indexed (5 = June)
...
if (today.getMonth() === 5 && today.getDate() === 11) { // Month is 0-indexed (5 = June)
    createConfetti();
}
...
document.getElementById('giftBtn').addEventListener('click', function (e) {
    ...
    setTimeout(() => {
        alert('Sürpriz! Doğum gününde en güzel dileklerimle! 🎁');
    }, 500);
});
